...
class AudioService {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // 2. Configurar modo de audio con valores compatibles
      ...
      console.log('🎙️ Hardware Audio Configurado (AEC activo)');
    } catch (error) {
    ...
    this.monitoringInterval = setInterval(async () => {
      ...
      try {
        ...
        if (status.isRecording && status.metering !== undefined) {
          // Aumentamos la sensibilidad convirtiendo el valor de metering de forma más agresiva
          const amplitude = Math.pow(10, status.metering / 20); // Usamos 20 en lugar de 10 para mayor rango dinámico
        ...
        // Silenciamos errores menores de polling
      }
    }, 30); // Bajamos a 30ms para máxima reactividad
  }
